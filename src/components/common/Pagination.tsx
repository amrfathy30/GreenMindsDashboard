...
const Pagination: React.FC<PaginationProps> = ({
  ...
  const navButtonClass =
    "flex items-center gap-2 px-2 py-1 text-sm font-medium transition-colors disabled:opacity-20 disabled:cursor-not-allowed";
    ...
    <div className="flex justify-center items-center gap-4 ">
      ...
      <div className="flex items-center gap-2">
        {generatePages(currentPage, totalPages).map((page, idx) =>
          page === "..." ? (
            <span key={`ellipsis-${idx}`} className="text-[#BDBDBD] px-1">
              ...
            </span>
          ) : (
            <button
