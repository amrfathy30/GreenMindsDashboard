...
export default function PointsDistribution({
  ...
  const originalColors = ["#F7B500", "#FF4906", "#39CEF3", "#72CA3D", "#9B51E0", "#FA5A9F"];
  const { t } = useLanguage();

  const sortedData = [...levelsStatsData].sort((a, b) => b.UsersCount - a.UsersCount);

  const series = sortedData.length > 0 ? sortedData.map((lvl) => lvl.UsersCount) : [0];
  const labels = sortedData.length > 0 ? sortedData.map((lvl) => lvl.LevelName) : ["No Data"];
  const chartColors = sortedData.map((_, i) => originalColors[i % originalColors.length]);
  ...
  const hasData = sortedData && sortedData.length > 0 && sortedData.some((lvl) => lvl.UsersCount > 0);
