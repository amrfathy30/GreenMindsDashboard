    ...
    <>
      ...
      <div className="relative rounded-2xl border-b border-[#D9D9D9] pb-5 dark:border-gray-800 min-h-[calc(100vh-48px)] flex flex-col dark:bg-neutral-800 bg-[#EDEDED]">
        <div className="h-auto min-h-[70px] mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 px-5 border-b border-[#D9D9D9] dark:border-gray-600 py-4 shrink-0">
          <h2 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white">{t("Videos - Admin")}</h2>
          {canAdd && (
            <Button size="sm" variant="primaryGrid" className="w-full sm:w-auto rounded-lg px-4 py-2 text-sm" onClick={() => { setSelectedVideo(null); setIsAddOpen(true); }}>
        ...
        {videos?.length === 0 && !loading ? <EmptyState title="No Data Found" description="Videos Section has no data yet!" /> : ""}
                ...
                {videos.map((video, index) => (
                  <tr key={video.Id} className={`border-b border-gray-50 dark:border-gray-800/50 transition-colors ${index % 2 === 0 ? "bg-white dark:bg-transparent" : "bg-[#D9D9D940] dark:bg-white/[0.01]"}`}>
                    <td className="px-4 py-3">
                      <div onClick={() => { setSelectedVideo(video); setIsPreviewOpen(true); }} className="relative aspect-[16/9] h-[45px] md:h-[55px] rounded-xl overflow-hidden cursor-pointer border border-gray-100 dark:border-gray-700">
                        <img src={video.ThumbnailUrl?.startsWith('http') ? video.ThumbnailUrl : `${BASE_URL}/${video.ThumbnailUrl}`} className="w-full h-full object-cover" alt="" />
                        <div className="absolute inset-0 flex items-center justify-center bg-black/5 hover:bg-black/15 transition-all">
                          <div className="bg-white p-1 rounded-full"><Play size={10} fill="#6B6B6B" className="text-[#6B6B6B]" /></div>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-sm md:text-base font-lalezar font-medium text-gray-900 dark:text-white line-clamp-1">{video.TitleEn} / {video.TitleAr}</span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <span className="text-[#00B69B] font-lalezar text-lg md:text-xl">+{video.NumberOfPoints}</span>
                        <img src="/images/video-thumb/Stack of Coins.svg" className="w-6 h-6 md:w-8 md:h-8" alt="" />
                      </div>
                    </td>
                    <td className="px-4 py-3 text-center">
