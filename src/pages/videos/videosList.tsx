...
export default function VideosList() {
  ...
  return (
    <>
      <PageMeta
        title="Videos Dashboard | Green minds Admin Dashboard"
        description="Manage your videos list easily."
      />
      ...
      <div className="relative rounded-2xl border-b border-[#D9D9D9] pb-5 bg-[#EDEDED]  dark:border-gray-800 dark:bg-[#adf4b514]  h-[calc(100vh-48px)]">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="">
                <th className="px-6 py-3 text-sm font-bold text-black uppercase tracking-wider">Thumbnail</th>
                <th className="px-6 py-3 text-sm font-bold text-black uppercase tracking-wider">Title</th>
                <th className="px-6 py-3 text-sm font-bold text-black uppercase tracking-wider text-center">Points</th>
                <th className="px-6 py-3 text-sm font-bold text-black uppercase tracking-wider text-center">Age group</th>
                <th className="px-6 py-3 text-sm font-bold text-black uppercase tracking-wider text-center">Actions</th>
              </tr>
            </thead>
            <tbody>
              {initialVideos.map((video, index) => (
                <tr key={video.id} className={`transition-colors ${index % 2 === 0 ? "bg-[rgba(217,217,217,0.25)] dark:bg-white/[0.02]" : "bg-[rgba(250,250,250,1)] dark:bg-transparent"}`}>
                  <td className="px-6 py-0">
                    <div onClick={() => handlePreview(video)} className="relative aspect-[16/9] h-[65px] rounded-2xl overflow-hidden bg-white dark:bg-gray-800 shadow-inner group cursor-pointer border border-gray-100 dark:border-gray-700">
                  ...
                  <td className="px-6 py-0">
                    <span className="text-base font-lalezar font-semibold text-[rgba(0,0,0,1)] dark:text-white leading-none">{video.title}</span>
                  </td>
                  <td className="px-6 py-0 text-center">
                    <div className="flex items-center justify-center">
                      <span className="text-[#00B69B] font-lalezar text-2xl">{video.points}</span>
                      <img src="/images/video-thumb/Stack of Coins.svg" alt="coins" className="w-14 h-15 object-contain mt-4" />
                    </div>
                  </td>
                  <td className="px-6 py-0 text-center">
